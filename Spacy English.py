...
def main(prefix, filelist, output):
    with io.open(output, 'w', encoding='utf-(..)') as out:
        for i, fname in enumerate(io.open(filelist, 'r', encoding='utf-(..)')):
            fname = fname.strip()
            doc = io.open('{0}/{1}'.format(prefix, fname), 'r', encoding='utf-(..)').read()
            spacy_doc = nlp(doc)
